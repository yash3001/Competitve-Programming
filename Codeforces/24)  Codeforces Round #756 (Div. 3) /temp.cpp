...
void dfs(ll n, vector<vector<ll>> &graph, vector<ll> &vis, vector<ll> &ans, vector<ll> &res){
    vis[n] = 1;
    for(const auto &c: graph[n]){
        if(!vis[c]){
            res[c-1] = ans[c-1] - ans[n-1];
            dfs(c, graph, vis, ans, res);
        }
    }
}
...
void solve(){
    ...
    dfs(root, graph, visited, ans, res);
    //deb(res);
    for(ll i=0; i<n; i++){
        if(res[i] < 0){
            cout<<-1<<endl;

...
void solve(){
    ...
    ll ans = 0;
    ll t = k;
    for(ll i=0; i<vpp.size();){
        t = k;
        if(vpp[i].second > k){
            if(vpp[i].second % k == 0){
                ll temp = vpp[i].second/k;
                ans += temp*vpp[i].first*2;
            }
            else{
                ll temp = (vpp[i].second/(k*1.0));
                ans += (temp-1)*2*vpp[i].first + vpp[i].first;
                t -= vpp[i].second % k;
            }
            i++;
            if(i == vpp.size()){
        ...
        while(t > 0 && i < vpp.size()){
            if(vpp[i].second <= t){
                t -= vpp[i].second;
                ans += vpp[i].first - vpp[i-1].first;
                i++;
            }
            else{
                break;

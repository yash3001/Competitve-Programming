...
void solve(){
    ...
    mn = LLONG_MAX;
    for(ll i=0; i<n; i++){
        mn = min(mn, abs(b[n-1]-a[i]));
    }
    ans = min(ans, abs(a[0]-b[0]) + abs(a[n-1]-b[0]) + mn);
    ...
    mn = LLONG_MAX;
    for(ll i=0; i<n; i++){
        mn = min(mn, abs(b[0]-a[i]));
    }
    ans = min(ans, abs(a[0]-b[n-1]) + abs(a[n-1]-b[n-1]) + mn);
    ...
    temp += mn;
    ans = min(ans, abs(a[0]-b[n-1]) + temp);

    // 0-m n-0
    temp=0;
    mn = LLONG_MAX;
    for(ll i=0; i<n; i++){
        mn = min(mn, abs(b[i]-a[0]));
    }
    temp += mn;
    mn = LLONG_MAX;
    for(ll i=0; i<n; i++){
    ...
    temp += mn;
    ans = min(ans, abs(a[n-1]-b[0]) + temp);
    ...
    temp += mn;
    ans = min(ans, abs(a[n-1]-b[n-1]) + temp);

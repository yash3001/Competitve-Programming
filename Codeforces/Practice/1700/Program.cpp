...
void solve(){
    ...
    vll pre(n+2,0), lmin(n+2,0), lmax(n+2,0);
    for(ll i=1; i<=n; i++){
        pre[i] = pre[i-1]+nums[i];
        lmax[i] = max(lmax[i-1], pre[i]);
        lmin[i] = min(lmin[i-1], pre[i]);
    }
    vll rmin(n+2,0), rmax(n+2,0);
    for(ll i=n; i>=1; i--){
        rmax[i] = max(0ll, rmax[i+1]+nums[i]);
        rmin[i] = min(0ll, rmin[i+1]+nums[i]);
    }
    deb(nums);
    deb(lmax);
    deb(lmin);
    ...
    while(m--){
        ll l,r; cin>>l>>r;
        ll maxx = max(lmax[l-1], pre[l-1]+rmax[r+1]);
        ll minn = min(lmin[l-1], pre[l-1]+rmin[r+1]);
        deb(lmax[l-1]);
        deb(pre[l-1]);
        deb(rmax[r+1]);

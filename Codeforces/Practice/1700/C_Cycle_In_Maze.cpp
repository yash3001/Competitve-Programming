...
void dfs(ll x, ll y, vvc &mat, vvll &vis, vvll &dist){
    vis[x][y] = 1;
    for(ll i=0; i<4; i++){
        ll nx = x+xadd[i];
        ll ny = y+yadd[i];
...
void solve(){
    ...
    vvll vis(n, vll(m, 0)), dist(n, vll(m, 0));
    ll x, y;
    for(ll i=0; i<n; i++){
        for(ll j=0; j<m; j++){
            if(mat[i][j] == 'X'){
    ...
    for(ll i=1; i<=k; i++){
        ...
        for(ll j=0; j<4; j++){
            ll nx = x+xadd[j];
            ll ny = y+yadd[j];
            if(nx >= 0 && nx < n && ny >= 0 && ny < m && vis[nx][ny] && dist[nx][ny] <= k-i){
                ans.pb(pos[j]);
                found = true;
                x = nx;
                y = ny;

...
void dfs(ll n, ll x, vvll &adj, vll &vis){
    vis[n] = 1;
    if(n == x){
        found = true;
        nodes.pb(n);
...
void ddfs(ll n, vvll &adj, vll &vis, vll &dist, ll d){
    vis[n] = 1;
    dist[n] = d;
    for(const auto &c: adj[n]){
        if(!vis[c]){
            ddfs(c, adj, vis, dist, d+1);
        }
    }
}
...
void solve(){
    ...
    ll i;
    ll ans = 0;
    for(i=0; i<ll(nodes.size()+1)/2; i++){
        vis[nodes[i]] = 1;
        ans += 2;
    }
    ll nd = nodes[i];
    vll dist(n+1, 0);
    ddfs(nd, adj, vis, dist, 0);
    ll d = *max_element(all(dist));
    ans += d*2;
    cout<<ans<<endl;
}

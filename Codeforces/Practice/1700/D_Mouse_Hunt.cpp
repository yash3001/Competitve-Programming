...
void fdfs(ll n, vvll &adj, vll &vis, vll &order){
    vis[n] = 1;
    for(const auto &c: adj[n]){
        if(!vis[c]){
            fdfs(c, adj, vis, order);
        }
    }
    order.pb(n);
}

void sdfs(ll n, vvll &adj, vll &vis, vll &nodes){
    vis[n] = 1;
    nodes.pb(n);
    for(const auto &c: adj[n]){
        if(!vis[c]){
            sdfs(c, adj, vis, nodes);
        }
    }
}

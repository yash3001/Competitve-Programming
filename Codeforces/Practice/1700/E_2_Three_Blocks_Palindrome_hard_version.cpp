...
void solve(){
    ...
    for(ll i=1; i<=n; i++){
        dist[a[i-1]].pb(i);
        ans = max(ans, ll(dist[a[i-1]].size()));
    }
    deb(dist);

    for(ll i=1; i<=200; i++){
        for(ll j=0; j<dist[i].size()/2; j++){
            ll x = dist[i][j] + 1;
            ll y = dist[i][dist[i].size()-j-1] - 1;
            for(ll z=1; z<=200; z++){
                ll cnt = pre[z][y] - pre[z][x-1];
                ans = max(ans, cnt + 2*(j+1));
            }
        }
    }
    deb(ans);

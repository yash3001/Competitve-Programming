...
void solve(){
    ...
    for(ll i=2; i<n; i++){
        for(ll j=i; j<n; j++){
            ll cnt1 = LLONG_MIN;
            for(ll k=1; k<=26; k++){
                ll cntf = pre[k][i-1] - pre[k][0];
                ll cnts = pre[k][n] - pre[k][j];
                cnt1 = max(cnt1, 2*min(cntf, cnts));
            }
            ll cnt2 = LLONG_MIN;
            for(ll k=1; k<=26; k++){
                cnt2 = max(cnt2, pre[k][j] - pre[k][i-1]);
            }

...
void dfs(ll i, ll j, bool &flag, vvll &adj, vll &vis, stack<ll> &st){
    vis[i] = 1;
    st.push(i);
    if(i == j){
        flag = true;
...
void solve(){
    ...
    for(ll i=1; i<=n; i++){
        for(ll j=i+1; j<=n; j++){
            ...
            for(ll k=1; k<path.size(); k++){
                vmp[{path[k], path[k-1]}] = 1;
                d += mp[{path[k], path[k-1]}];
            }
            deb(d);
            each(p, mp){
                if(vmp[{p.first.first, p.first.second}] || vmp[{p.first.second, p.first.first}]){
                    continue;
                }
                deb(p);
                d += 2*p.second;
                vmp[{p.first.second, p.first.first}] = 1;
                vmp[{p.first.first, p.first.second}] = 1;
            }
            ans = min(ans, d);
        }
    }

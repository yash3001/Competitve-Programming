...
void solve(){
    ...
    for(ll z=0; z<k; z++){
        ll sum = accumulate(all(vec[z]), 0LL);
        for(ll i=0; i<vec[z].size(); i++){
            ll x = vec[z][i];
            mp[sum-x].insert(z);
            if(mp[sum-x].size() > 1){
                ind1 = *(mp[sum-x].begin());
                ind2 = *(mp[sum-x].rbegin());
                fsum = sum-x;
                break;
            }
        }
    ...
    cout<<"YES"<<endl;
    cout<<ind1+1<<" ";
    ll sum1 = accumulate(all(vec[ind1]), 0LL);
    for(ll i=0; i<vec[ind1].size(); i++){
        if(sum1 - vec[ind1][i] == fsum){
            cout<<i+1<<endl;
            break;
        }
    }
    cout<<ind2+1<<" ";
    ll sum2 = accumulate(all(vec[ind2]), 0LL);
    for(ll i=0; i<vec[ind2].size(); i++){
        if(sum2 - vec[ind2][i] == fsum){
            cout<<i+1<<endl;
            break;
        }
    }

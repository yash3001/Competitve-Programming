...
/* ------------------Binary Search------------------ */
// 1) Lower Bound -> returns itera(tor to the first element greater than or equal to the given element or returns end() if no such element exists
...
void dfs1(ll n, vvll &adj, vll &vis, vll &col){
    vis[n] = 1;
    for(const auto &nd: adj[n]){
        if(!vis[nd]){
            if(col[nd] != col[n]){
                n1 = nd; n2 = n;
                // dfs1(nd, adj, vis, col);
...
bool dfs(ll n, vvll &adj, vll &vis, vll &col, ll &r){
    vis[n] = 1;
...
void solve(){
    ...
    if(dfs(n1, adj, vis1, col, n1)){
        cout<<"YES"<<endl;
        cout<<n1+1<<endl;
        return;
    }
    if(dfs(n2, adj, vis2, col, n2)){
        cout<<"YES"<<endl;
        cout<<n2+1<<endl;
        return;
    }

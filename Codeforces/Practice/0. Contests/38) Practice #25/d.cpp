...
void solve(){
    ...
    if(check1.size()%2 == 0 && check2.size()%2 != 0 || check1.size()%2 != 0 && check2.size()%2 == 0){
        cout<<-1;
        return;
    }
    else{
        cout<<check1.size()/2+check2.size()/2+(check1.size()%2+check2.size()%2)<<endl;
        deb(check1);
        deb(check2);
        deb(check1.size())
        if(check1.size() != 0)
        for(ll i=0; i<check1.size()-1; i++){
            cout<<check1[i]+1<<" "<<check1[i+1]+1<<endl;
            i++;
        }
        if(check2.size() != 0)
        for(ll i=0; i<check2.size()-1; i++){
            cout<<check2[i]+1<<" "<<check2[i+1]+1<<endl;
            i++;
        }
        if(check2.size()&1){
            cout<<check2[check2.size()-1]+1<<" "<<check2[check2.size()-1]+1<<endl;
            cout<<check1[check1.size()-1]+1<<" "<<check2[check2.size()-1]+1<<endl;
        }
    }
}

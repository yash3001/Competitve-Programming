...
void solve(){
    ll n; cin>>n;
    vll arr(200000), des(200000), sum(200000), vis(200000, 0);
    for(ll i=1; i<=n; i++){
        cin>>arr[i];
    }
    for(ll i=1; i<=n; i++){
    ...
    ll mx = 0;
    for (ll i = n; i>1; i--){
        int idx = des[i]; vis[idx] = 1;
        if(vis[idx + 1]){
            sum[find(idx + 1)] += sum[find(idx)];
            parent[find(idx)] = find(idx + 1);
        }
        if(vis[idx - 1]){
            sum[find(idx - 1)] += sum[find(idx)];
            parent[find(idx)] = find(idx - 1);
    	}
        ans.pb(max(mx, sum[find(idx)]));

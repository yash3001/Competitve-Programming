...
void solve(){
    ...
    while(!q.empty()){
        ...
        for(ll k=0; k<4; k++){
            ...
            if(new_x >= 0 && new_x < n && new_y >= 0 && new_y < n){
                if(!vis[new_x][new_y] && mat[new_x][new_y] == 1){
                    vis[new_x][new_y] = 1;
                    q.push({new_x, new_y});
                }
            }
    ...
    while(!q1.empty()){
        ...
        for(ll k=0; k<4; k++){
            ...
            ll new_y = y+yadd[k];
            if(new_x >= 0 && new_x < n && new_y >= 0 && new_y < n){
                if(!vis1[new_x][new_y] && mat[new_x][new_y] == 1){
                    vis1[new_x][new_y] = 1;
                    q1.push({new_x, new_y});
                }
            }
        ...
        vis1[r2][c2] = 1;
        while(!q2.empty()){
            ll x1 = q2.front().first;
            ll y1 = q2.front().second;
            ans = min(ans, ((x-x1)*(x-x1))+((y-y1)*(y-y1)) );
            q2.pop();
            for(ll k=0; k<4; k++){
                ll new_x1 = x1+xadd[k];
                ll new_y1 = y1+yadd[k];
                if(new_x1 >= 0 && new_x1 < n && new_y1 >= 0 && new_y1 < n){
                    if(!vis2[new_x1][new_y1] && mat[new_x1][new_y1] == 1){
                        vis2[new_x1][new_y1] = 1;
                        q2.push({new_x1, new_y1});
                    }
                }
            }
            // deb(x);

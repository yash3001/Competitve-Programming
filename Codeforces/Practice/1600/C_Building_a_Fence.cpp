...
void solve(){
    ...
    vp.push_back({heights[0], heights[0]});
    for(ll i=1; i<n; i++){
        ll prev_max = vp[i-1].second;
        ll prev_min = vp[i-1].first;
        ll new_min = max(prev_min-(k-1), heights[i]);
        ll new_max = min(prev_max+k-1, heights[i]+k-1);
        if(new_min > new_max){
            cout<<"NO"<<endl;
            return;
        }
        vp.push_back({new_min, new_max});
    }
    deb(vp)
    if(heights[n-1] >= vp[n-1].first && heights[n-1] <= vp[n-1].second){
        cout<<"YES"<<endl;
    }
    else{
        cout<<"NO"<<endl;

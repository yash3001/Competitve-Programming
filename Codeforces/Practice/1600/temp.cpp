...
bool dfs1(ll root,ll parent,ll xr,ll vis){
	ll check2 = (sum[root]);
	ll check = (sum[1]^check2)^xr;
	if(check2==check && root!=1 ){
		if(timeIn[root] <= timeIn[vis] && timeOut[vis] <= timeOut[root]){
...
bool dfs2(ll root,ll parent,ll xr){
	if(sum[root]==xr && root!=1){
		if(dfs1(1,0,xr,root)){
			return true;
		}
	}

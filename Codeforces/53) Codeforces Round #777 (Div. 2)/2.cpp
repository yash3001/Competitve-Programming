...
vector<pair<ll, ll>> centres;
unordered_map<string, ll> age, category;
ll m, n;
...
void solve(){
    ...
    for(ll i=0; i<n; i++){
        ...
        ll dist = LLONG_MAX;
        for(ll j=0; j<m; j++){
            dist = min(dist, abs(x-centres[j].first)+abs(y-centres[j].second));
        }
        for(ll j=0; j<m; j++){
            ll dt = abs(x-centres[j].first)+abs(y-centres[j].second);
            if(dist == dt){
                ans[j].pb(s);
                break;
            }

...
void solve(){
    ...
    q.push({indx, indy});
    for(ll i=0; i<4; i++){
        ll new_x = indx+xadd[i];
        ll new_y = indy+yadd[i];
        if(new_x >= 0 && new_x < n && new_y >= 0 && new_y < m && mat[new_x][new_y] == '.'){
            ll cnt = 0;
            for(ll j=0; j<4; j++){
                ll new_x1 = new_x+xadd[j];
                ll new_y1 = new_y+yadd[j];
                if(new_x1 >= 0 && new_x1 < n && new_y1 >= 0 && new_y1 < m && mat[new_x1][new_y1] == '.'){
                    cnt++;
                }
            }
            deb(cnt);

...
vector<vector<ll> > findPowerSet(vector<ll>& nums)
{
...
void solve(vvll &subs){
    ll n; cin>>n;
    ll mn = block(n);

    for(ll i=0; i<subs.size(); i++){
        ll sm = 0;
        for(ll j=0; j<subs[i].size(); j++){
            sm += subs[i][j];
        }
        if(sm < n)
            mn = min(mn, block(n-sm)+ll(subs[i].size()));
        if(sm == n){
            mn = min(mn, ll(subs[i].size()));
        }
    }
    cout<<mn<<endl;
}
...
int main(){
    ...
    ll k = 1;
    ll i = 2;
    while(k <= 1000000000001){
        temp.pb(k);
        k *= i++;
    }
    sort(all(temp));
    vvll subs = findPowerSet(temp);
    ll t=1;
    cin >> t;
    for(ll i=1; i<=t; i++){
        pt(i);

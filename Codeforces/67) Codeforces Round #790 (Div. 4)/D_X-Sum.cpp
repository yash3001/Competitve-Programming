...
void solve(){
    ll n, m; cin>>n>>m;
    vvll mat(n, vll(m)); each(r, mat) daalo(r);

    ll ans = LLONG_MIN;
    F0(i, n){
        F0(j, m){
            ll t = mat[i][j];
            for(ll x = i-1, y = j-1; x >= 0 && y >= 0; x--, y--) t += mat[x][y];
            for(ll x = i-1, y = j+1; x >= 0 && y < m; x--, y++) t += mat[x][y];
            for(ll x = i+1, y = j+1; x < n && y < m; x++, y++) t += mat[x][y];
            for(ll x = i+1, y = j-1; x < n && y >= 0; x++, y--) t += mat[x][y];
            ans = max(ans, t);
        }
    }
    cout<<ans<<endl;

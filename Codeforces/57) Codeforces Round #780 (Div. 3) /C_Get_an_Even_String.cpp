...
// 4) __builtin_parity(x) -> returns the parity of the number of ones in the bit representaton of x.
    ...
    ll rec(string &, int, int, map<char,vll> &);
  ...
  void solve(){
    ...
    ll rec(string &s, int i, int ev, map<char,vll> &mp){
      if(i==s.size()){
        if(ev){
          return LLONG_MAX;
        }
      ...
      if(ev){
        return dp[i][ev]= rec(s,i+1,0,mp);
      }
      else{
        ll ind = upper_bound(all(mp[s[i]]),i)-mp[s[i]].begin();
        if(ind==mp[s[i]].size()){
          return dp[i][ev]=1+rec(s,i+1,ev,mp);
        }
        ind = mp[s[i]][ind];
        ll cur = ind-i-1;

        return dp[i][ev]=min(1+rec(s,i+1,ev,mp), cur+rec(s,ind,1,mp));
      }
      return 0;
    }

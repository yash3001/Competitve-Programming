...
void solve(){
    ...
    ll maxup = LLONG_MAX, maxdown = LLONG_MIN, maxleft = LLONG_MAX, maxright = LLONG_MIN;
    ll cnt = 0;
    for(ll i=0; i<n; i++){
        for(ll j=0; j<n; j++){
            if(mat[i][j] == 1){
                maxup = min(maxup, i);
                maxdown = max(maxdown, i);
                maxleft = min(maxleft, j);
    ...
    ll ans = 0;
    for(ll i=maxup; i<=maxdown; i++){
        for(ll j=maxleft; j<=maxright; j++){
            if(mat[i][j] != 1){
                ans++;
            }
        }

...
bool dfs(ll n, vvll &adj, vll &vis, string &s, vs& b){
    deb(n)
    if(vis[n]){
        return false;
    }
    ...
    if(adj[n].size() == 1){
        string c1 = b[adj[n][0]];
        // s += c1;
        if(!dfs(adj[n][0], adj, vis, s, b)){
            return false;
        }
    }
    else if(adj[n].size() == 0){
        return true;
    }
    else{
        string c1 = b[adj[n][0]];
...
void solve(){
    ...
    for(ll i=0;i<n;){
        ...
        i++;
        while(i < n && v[i].front() == v[i].back() && v[i].front() == ch){
            s += v[i];
            i++;
        }
        b.push_back(s);

...
void solve(){
    int r,c,R; cin>>c>>r>>R;
    int ncycles=min(r,c)/2;
    int m[r*c], mat[150][1196], i,x,y, temp,len;
    for(i=0;i<r*c;i++)cin>>m[i];
    for(temp=0;temp<ncycles;temp++){
        i=0;
        x=y=temp;
        for(;y<c-temp-1;y++)
            mat[temp][i++]=m[r*y+x];
    ...
    for(temp=0;temp<ncycles;temp++){
        len=2*(r-2*temp)+2*(c-2*temp)-4;
        i=-R%len+len;
        x=y=temp;
        for(;y<c-temp-1;y++)
            m[r*y+x]=mat[temp][i++%len];
        for(;x<r-temp-1;x++)
            m[r*y+x]=mat[temp][i++%len];
        for(;y>temp;y--)
            m[r*y+x]=mat[temp][i++%len];
